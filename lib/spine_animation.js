...
SpineAnimation.prototype.initialize = function(skeleton_name) {
    ...
    this.image = ContentManager.images[this.skeleton_name].image;
...
SpineAnimation.prototype.play = function(animation_name, loop, flipped, callback) {
    ...
    if (flipped) {
        this.image = ContentManager.images[this.skeleton_name + '_fliped'].image;
    } else {
        this.image = ContentManager.images[this.skeleton_name].image;
    }
...
//    }
    ...
    if (this.is_visible) {
        ...
        apos.sub(ach);
        context.translate(Math.round(apos.x), Math.round(apos.y));
